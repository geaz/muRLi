...
namespace Murli
{
    void mjsSetSource(struct mjs *mjs)
    {
        mjs_val_t source_delay = mjs_arg(mjs, 0);
        int32_t source = mjs_get_int32(mjs, source_delay);

        ScriptContextPointer->source = static_cast<Client::AnalyzerSource>(source);
    }
