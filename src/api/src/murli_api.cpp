#include <iostream>
#include "murli_api.hpp"

namespace Murli
{
    namespace Api
    {
        MurliApi::MurliApi(std::string url) : _websocket(url)
        { 
        ...
        void MurliApi::onConnection(const bool connected)
        {
            if(connected) _freqAnalyzer.start();
            else _freqAnalyzer.stop();
        }

        void MurliApi::onCommandReceived(const Client::Command command)
        {
        ...
        void MurliApi::onFrequencyCalculated(const float decibel, const uint8_t volume, const uint16_t dominantFrequency, const std::array<uint8_t, BAR_COUNT> buckets)
        {
        ...
        Server::Command MurliApi::createAnswer(const Client::Command command, const Server::CommandType answerCommandType) const
        {
