...
static inline unsigned StringGetLen(TinyString s) { return (unsigned)s.len_; }
static inline const char *StringGetPtr(TinyString s) { return (const char *)(intptr_t)s.ptr_; }
#ifdef SMALL_PTRS
static inline void StringSetLen(TinyString *s, unsigned len) { s->len_ = (uint16_t)len; }
static inline void StringSetPtr(TinyString *s, const char *ptr) { s->ptr_ = (uint16_t)(intptr_t)ptr; }
#else
static inline void StringSetLen(TinyString *s, unsigned len) { s->len_ = len; }
static inline void StringSetPtr(TinyString *s, const char *ptr) { s->ptr_ = ptr; }
#endif
...
typedef struct ufunc {
    TinyString body; // pointer to the body of the function
    int nargs;   // number of args
    // names of arguments
    TinyString argName[MAX_BUILTIN_PARAMS];
} UserFunc;
